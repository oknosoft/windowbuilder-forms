...
class FakePrmRow {
  ...
  isDiff() {
    const {_raw: {cnstr, value}, _rows} = this;
  ...
  set value(v) {
    ...
    if(this._raw.cnstr) {
      ...
      if(value == row0?.value) { // если значение в слое равно значению в изделии
        if(row0) {
          if(rowc) {
...
function groupedRows({ts, cnstr, inset, layer, sys}) {
  const raw = new Map();
  const aprm = layer?.furn_set?.used_params();
  ts.find_rows({cnstr, inset, hide: false}, (prow) => {
    const {param} = prow;
    if(!raw.has(param)) {
      raw.set(param, new FakePrmRow({ts, cnstr, param, inset, value: prow.value}));
    }
    raw.get(param).add(prow);
  });
  if(aprm) {
    for(const param of aprm) {
      const topRow = ts.find({cnstr: 0, param, inset, hide: false});
      if(topRow) {
        if(!raw.has(param)) {
          raw.set(param, new FakePrmRow({ts, cnstr, param, value: topRow.value}));
        }
  ...
  const grouped = [];
  const {job_prm: {lists: {sys_prm_grp_names}}, utils} = $p;
  for(const [param, prow] of raw) {
    let grp = 0
    if(sys_prm_grp_names?.length && sys) {
      const row = sys.product_params.find({param}) || sys.furn_params.find({param});
      if(row) {
        grp = row.grouping;
      }
    }
...
export default function LinkedProps({ts, cnstr, inset, layer, project}) {
  ...
  const grouped = groupedRows({ts, cnstr, inset, layer, sys});
  if(!grouped.length) {
    return null;
  }
  else if(grouped.length === 1) {
    return children({rows: grouped[0].rows, layer, project, fields, sys, grid, _owner});
  }
  else {
    return grouped.map(({name, rows}, index) => {
      const elements =  children({rows, layer, project, fields, sys, grid, _owner});
      return name ?
      <FieldSet key={`grp-${index}`} title={name}>{
        elements
      }</FieldSet> : elements;
    });
  }
